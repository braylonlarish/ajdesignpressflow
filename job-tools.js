...
setTimeout(() => {
  ...
  const searchRequest = raw => {
    const match = String(raw || '').trim().match(/^(?:search|find|look\s+up|lookup)\s+(?:for\s+)?(?:part\s+|job\s+|product\s+)?(.+)$/i);
    return match && match[1].trim() ? match[1].trim() : null;
  };

  // Handle a search at the form first. This keeps job search available even
  // if the optional online AI service is unavailable or responds with an error.
  document.querySelector('#bendbot-form').addEventListener('submit', event => {
    const query = searchRequest(document.querySelector('#bendbot-input').value);
    if (!query) return;
  ...
  bendbotReply = function (request) {
    ...
    // Accept natural requests such as "search PX-8810" and "search part PX-8810"
    // before falling back to the optional AI connection.
    const searchMatch = searchRequest(raw);
    if (searchMatch) return openSearch(searchMatch);
